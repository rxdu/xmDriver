 ...
 * ring_buffer.hpp
 *
 * Byte-stream ring buffer for transport protocol parsing: span writes,
 * drain reads, Peek/PeekAt + partial consume (frame resync), Reset, and an
 * internal mutex tolerating multiple sender threads.
 *
 * Repatriated from xmbase/container at xmBase 0.5.0 (ADR 0007 placement
 * rule: one consumer -> lives with the consumer; the W1 fit audit found
 * this parser-shaped API deliberately does NOT generalize to the family's
 * record-handoff queues). The foundation's record-store roles are served
 * by xmbase/concurrency (MessageSlot / MessageBuffer / SpscQueue).
 *
